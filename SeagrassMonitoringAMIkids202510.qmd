---
title: "Report: 2026 Seagrass Monitoring Training"
...
df <- data.frame(
  lng = c(-82.596, -82.614),
  lat = c(27.854, 27.87),
  site = c("May-June 2026 Sampling Area", "AMIkids Marina")
)
...
A combination of approximately 10 students and adults attended each of the workshops. The students were from AMIkids programs in Hillsborough and Pinellas Counties. Adults were volunteers from Enterprise (June 11 and Jabil (June 12), as well as AMIkids staff and volunteers. Instructors included AMIkids staff and a contractor.

An instructional slide show was presented and gave background information on AMIkids, boat safety, and seagrass ecology. Transport was provided on the AMIkids flagship vessel, “Orlando’s Rose”, a 65-foot long landing craft jet-propelled boat. Due to shallow water conditions, participants were ferried from the Orlando's Rose to the seagrass flat via a pontoon boat.
...
The group was then divided into 2-4 teams that conducted quadrat assessments at a series of pre-established quadrats. The quadrats were placed each day with no intention for repeated observations.

**Results**

Each team completed 2-4 quadrats during the allotted time (approximately 1 hour). Secchi readings, measured in over 3m water, ranged from 260-320 cm and averaged xxx cm over the 5-day period (@fig-Secchi). Depth of quadrats ranged from XX-XXX cm, with a mean depth of xx.x cm (@fig-depth) with the mean daily depth decreasing over the week. Sediment generally varied from sand to muddy sand, although instances of shelly sand and mud were observed (@tbl-sed).

```{r}
#| label: fig-Secchi
#| fig-cap: "Secchi depths measured during 2026."

# create the secchi depth bar chart
secchi <- read.csv("https://raw.githubusercontent.com/GmangroveBari/Seagrass2026-AMIkids/refs/heads/main/2026-WaterQualityData.csv")

# make a ggplot
secchi_fig <- ggplot(secchi, aes(x = Team, y = Secchi.cm)) + geom_bar(stat = 'identity', position = 'dodge') + geom_col(fill = "steelblue")
...
```{r}
#| label: fig-depth
#| fig-cap: "Mean depths for 2026 sampling events."
#| fig-align: "center"

# create the avg depth bar chart
depth <- read.csv("https://raw.githubusercontent.com/GmangroveBari/Seagrass2026-AMIkids/refs/heads/main/2026-AMIkidsSeagrassClasses.csv")

#data summary and graph: date, species, and abundance
  Mean_Depth <- depth %>% 
    drop_na(Depth) %>%
    group_by(Date) %>%
...
# make a ggplot
Depth_fig <- ggplot(Mean_Depth, aes(x = Date, y = AvgD)) + 
...
sed <- read.csv("https://raw.githubusercontent.com/GmangroveBari/Seagrass2026-AMIkids/refs/heads/main/2026-AMIkidsSeagrassClasses.csv")
...
Braun-Blanquet abundance varied both daily and within each day (@fig-BB). All three common seagrass species were recorded. Manatee grass (*Syringodium testudinum*) had the highest overall abundance of the three species (@tbl-means).

```{r}
#| label: fig-BB
#| fig-cap: "Braun-Blanquet Coverage at Weedon Island in 2026."

Seagrass1 <- read.csv("https://raw.githubusercontent.com/GmangroveBari/Seagrass2026-AMIkids/refs/heads/main/2026-AMIkidsSeagrassClasses.csv")
...
#removing empty rows - no SAV observed  
  NoZeroAb <- mean_AbApp[apply(mean_AbApp!=0, 1, all),] %>% 
    drop_na(Date, Species, AvgAbu)

#plotting abundance data in grouped bar chart
NoZeroAb %>%
  plot_ly(x = ~Date, y = ~AvgAbu, color = ~Species) %>%
  add_bars() %>%
  layout(barmode = "group",  xaxis = list(title = "Date", showgrid=T),
         yaxis = list(title = "Braun-Blanquet Coverage"))

```

Overall general appearance (@tbl-means), based on characteristics such as the amount of necrotic leaf material and broken edges, averaged between a score of 1 (excellent) and fair (3) for all species.

```{r}
#| label: tbl-means
#| tbl-cap: "Overall averages for several different seagrass metrics assessed in 2026. Appearance and Epiphyte Density were scored on an objective scale from 1 (Excellent/None) to 5 (Poor/Heavy)."

Table2a <- read.csv("https://raw.githubusercontent.com/GmangroveBari/Seagrass2026-AMIkids/refs/heads/main/2026-AMIkidsSeagrassClasses.csv")
...
\<<Update as needed>\> Estimates of epiphyte coverage varied widely over the year (@fig-epiphyte) but overall means were similar for all three species (@tbl-means).

```{r}
#| label: fig-epiphyte
#| fig-cap: "Mean epiphyte density at Weedon Island in 2026."


Epiphyte <- read.csv("https://raw.githubusercontent.com/GmangroveBari/Seagrass2026-AMIkids/refs/heads/main/2026-AMIkidsSeagrassClasses.csv")
...
Mean_Epi %>%
  plot_ly(x = ~Date, y = ~AvgEpi, color = ~Species) %>%
  add_bars() %>%
  layout(barmode = "group", xaxis = list(title = "Date", showgrid=T),
